...
- Contacts in theme settings defaults: WhatsApp `+971 56 878 8789`, phone `+971 50 858 8828`, email `info@nerocasa.com`. Factory: Industrial Area 15, Sharjah. Delivery: UAE-wide.
...
A custom, production Shopify theme with a dark gold-on-black luxury visual layer in one stylesheet (`assets/nerocasa.css.liquid`), JSON templates, and a small catalog of nine marble products. Storefront design was accepted and published live 2026-09-17 (NeroCasa/main `#161950105824`). Storefront JS is two deferred files (header/cart/cursor/parallax + catalog search + scroll reveal). Google tag `GT-PJ46R9SC` loads from `layout/theme.liquid` after `<head>`. No Meta/TikTok pixels. Apps may still inject via `{{ content_for_header }}`. `/collections` uses a marble hero and a single The 9 tile. Agent system: `AGENTS.md`, `docs/`, `.cursor/skills/`, always-on rule `.cursor/rules/always-use-skills.mdc`.
